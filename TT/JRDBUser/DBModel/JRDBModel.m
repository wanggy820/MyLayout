...
@implementation JRDBModel

#pragma mark - override method
+ (void)initialize {
    if (self != [JRDBModel self]) {
        [self createTable];
    }
}
...
+ (NSDictionary *)getPropertys {
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([propertyType hasPrefix:@"T@\"NSString\""]) {
            ...
            NSArray *arr = [propertyType componentsSeparatedByString:@","];
            if (arr.count == 0) {
            ...
            NSString *className = [string substringWithRange:NSMakeRange(3, string.length - 4)];
            
            [proTypes addObject:className];
        }
...
- (BOOL)save {
    ...
    for (int i = 0; i < self.columeNames.count; i++) {
        ...
        value = [self valueToJson:value];
        
        [insertValues addObject:value];
    }
    
    if (self.columeNames.count > 1) {
...
- (id)valueToJson:(id)value {
    ...
    if ([value isKindOfClass:[NSArray class]]) {
        NSError *error;
        NSArray *dictArray = [[self class] mj_keyValuesArrayWithObjectArray:value];
...
//创建model对象
+ (JRDBModel *)findModel:(FMResultSet *)resultSet {
    JRDBModel *model = [[self.class alloc] init];
    for (int i=0; i< model.columeNames.count; i++) {
        ...
        } else if ([columeType isEqualToString:SQLINTEGER] || [columeType isEqualToString:[NSString stringWithFormat:@"%@ %@",SQLINTEGER,PrimaryKey]]) {
            [model setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
        } else if ([columeType isEqualToString:SQLREAL]) {
            [model setValue:[resultSet stringForColumn:columeName] forKey:columeName];
        } else {
            NSString *json = [resultSet stringForColumn:columeName];
            Class class = NSClassFromString(columeType);
            id value = [json mj_JSONObject];
            if (class == [NSMutableDictionary class] || class == [NSDictionary class]) {
                value = [NSMutableDictionary dictionaryWithDictionary:value];
            } else if (class == [NSMutableArray class] || class == [NSArray class]) {
                NSDictionary *dict = [[self class] objectClassInArray];
                Class arrayClass = dict[columeName];
                if (arrayClass) {
                    value = [arrayClass mj_objectArrayWithKeyValuesArray:value];
                } else {
                    value = [NSMutableArray arrayWithArray:value];
                }
            } else {
                value = [class mj_objectWithKeyValues:value];
            }
...
+ (BOOL)saveObjects:(NSArray *)array {
    //判断是否是jrBaseModel的子类
    for (JRDBModel *model in array) {
        if (![model isKindOfClass:[JRDBModel class]]) {
            return NO;
        }
    }
    ...
    [jrDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JRDBModel *model in array) {
            ...
            for (int i = 0; i < model.columeNames.count; i++) {
                ...
                value = [model valueToJson:value];
                [insertValues addObject:value];
            }
            if (model.columeNames.count > 1) {
                [keyString deleteCharactersInRange:NSMakeRange(keyString.length - 1, 1)];
                [valueString deleteCharactersInRange:NSMakeRange(valueString.length - 1, 1)];
            }
...
+ (NSArray *)findAll {
    ...
    [jrDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            JRDBModel *model = [self findModel:resultSet];
